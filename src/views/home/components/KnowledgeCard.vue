...
<template>
  <div class="knowledge-card van-hairline--bottom">
    ...
    <div class="body">
      <h3 class="title van-ellipsis">{{ item.creatorTitles }}</h3>
      <p class="tag">
        <span v-for="tag in item.topics" :key="tag"># {{ tag }}</span>
      </p>
      <p class="intro van-multi-ellipsis--l2">
        {{ item.content.replace(/<[^>]+>/g, '') }}
      </p>
      <div class="imgs">
        <van-image fit="cover" v-for="url in item.coverUrl" :key="url" :src="url" />
      </div>
      <p class="logs">
